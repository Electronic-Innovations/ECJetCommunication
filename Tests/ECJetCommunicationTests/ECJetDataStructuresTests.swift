...
final class ECJetDataStructuresTests: XCTestCase {
    ...
    func testPrintCount() throws {
        ...
        XCTAssertEqual(printCount.bytes, [0x02,0x0C,0x00,0x00,0x00])
        ...
        let printCount2 = try! PrintCount(bytes: getFrame!.data)
        XCTAssertEqual(printCount2.type, nil)
        XCTAssertEqual(printCount2.count, 418)
        XCTAssertEqual(printCount2.bytes, [0xA2,0x01,0x00,0x00])
    ...
    func testCorruptPrinterStatus() throws {
        let frame = Frame(bytes: [0x7e,0x1,0xf,0x0,0xc,0x0,0x6,0x0,0x0,0x0,0x0,0x0,0x0,0x2,0x19,0x0,0x0,0x0,0xc9,0xd,0x7f], verificationMethod: .crc16)
        let status = try! PrinterStatus(bytes: frame!.data)
        XCTAssertEqual(status.status, .jetStart)
        XCTAssertEqual(status.warningStatus, 25)
    }
